...
func listarAllClientesCars(w http.ResponseWriter, r *http.Request) {
	...
	data_clientes_cars := orm.NewQuerys("taxi_clientescars").Select("c_plac,l_marc,l_mode,l_color").OrderBy("c_plac").Exec().All()
...
func listOneClienteCar(w http.ResponseWriter, r *http.Request) {
	...
	//TODO: SELECT ALL DAT FROM CLIENTES
	data_clientes_cars := orm.NewQuerys("taxi_clientescars").Select("c_plac,l_marc,l_mode,l_color").Where("c_plac", "=", c_plac).Exec().One()
